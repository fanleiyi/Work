...
public class FindActivity extends Activity {
    ...
    public class MyLocationListener implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            if (locType==61||locType==65||locType==65||locType==161){
                // 定位成功
            ...
            MyApp.myLatLng=location1;
            //2)在location所对应的经纬度插上一个标志物

            MarkerOptions option = new MarkerOptions();
            option.position(location1);
            option.icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_locate));
            baiduMap.addOverlay(option);

            //3)移动屏幕的中心点到location所对应的位置
            MapStatusUpdate msu = MapStatusUpdateFactory.newLatLng(location1);
            baiduMap.animateMapStatus(msu);
            ...
            tv.setTextSize(TypedValue.COMPLEX_UNIT_SP,14);
            InfoWindow infoWindow=new InfoWindow(tv,location1,-50);
            baiduMap.showInfoWindow(infoWindow);
